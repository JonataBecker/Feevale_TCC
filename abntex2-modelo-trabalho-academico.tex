...
O cálculo da distribuição de níveis cor de uma imagem é denominado histograma (Figura \ref{fig:histograma}). O histograma de uma imagem, normalmente é representado de maneira gráfica, onde é indicada a quantidade de \textit{pixels} para cada nível de cor. Através do histograma de uma imagem é possível obter medidas estatísticas como os valores mínimo e máximo, valor médio, variância e o desvio padrão \cite{gonzalesWoods:2008}. Na Figura 1.1 é demonstrado o histograma de um segmento de uma imagem. 

Segundo Conci, Azevedo e Leta (2008), o histograma pode ser obtido da seguinte forma: analisa-se o tom de cada pixel; realiza-se a contagem do número de \textit{pixels} de cada intensidade de cor; representa-se esses valores na forma gráfica, correspondendo a frequência de cada tom.
...
Na Figura \ref{fig:limiarizacao} é aplicado um \textit{Threshold} de 190, desta forma, os pixels com valor maior que 190 recebem a cor branca, enquanto os \textit{pixels} com valor inferior recebem a cor preta. A imagem resultante será composta por \textit{pixels} pretos e brancos variando, sendo 0 ou 255.A implementação em java para o algoritmo pode ser acessado através do link:
...
O conceito de brilho está relacionado com a intensidade luminosa de uma fonte, quando é trabalhado com imagens digitais. Esta intensidade está relacionada com os \textit{pixels} da imagem. O contraste está relacionado com a medida de variação da intensidade luminosa por unidade de área \cite{gonzalesWoods:2008}.
...
Operações geométricas são aquelas que possuem o objetivo de permitir o mapeamento entre posições espaciais da imagem e a imagem modificada. Estas transformações consistem em aplicar operações responsáveis pela reorganização dos \textit{pixels} da imagem \cite{pedriniSchwartz:2008}. São operações que modificam os tons dos pixel na posição (\(x_o, y_o\)) da imagem de origem para outra imagem (\(x_d, y_d\))de destino, alterando, desta forma, a posição dos \textit{pixels} \cite{conciAzevedoLeta:2008}.
...
O processo de transladar um objeto consistem em deslocar ou somar a cada um dos \textit{pixels} da imagem um determinado valor (\(tx,ty,tz\)) \cite{conciAzevedoLeta:2008}. Segundo Pedrini e Schwartz (2008), a translação de uma imagem, utilizando o deslocamento \(tx\),\(ty\) e \(tz\), pode ser expressa na forma matricial por:
...
O processo de mudança de escala (zoom in, zoom out) consiste na multiplicação de cada um dos \textit{pixels} da imagem por um uma escala (\(s_x\), \(s_y\), \(s_z\))  \cite{conciAzevedoLeta:2008}. Segundo Pedrini e Schwartz (2008), a mudança de escala de uma imagem, utilizando os fatores \(s_x\), \(s_y\) e \(s_z\), pode ser expressa na forma matricial por:
...
Este filtro realiza a ordenação das intensidades dos \textit{pixels} existentes dentro da máscara, utilizando como valor para o pixel analizado o valor central dos elementos ordenados \cite{conciAzevedoLeta:2008}.
...
Filtros Gaussianos possuem características que são úteis para o processamento de imagens. As funções Gaussianas são simétricas, o que significa que o grau de suavização é aplicado da mesma forma em todas as direções (isotrópico). A suavização da imagem é obtida através da substituição de cada pixel da imagem pela média ponderada dos \textit{pixels} vizinhos \cite{pedriniSchwartz:2008}. 
...
O operador Frei-Chein utiliza um conjunto de nove máscaras de tamanho 3 x 3. As máscara M1 a M4 são utilizadas para realizar a detecção de bordas, já a M5 a M8 realizam a detecção de retas, e a M9 representa uma médias dos \textit{pixels} na região da matriz \cite{pedriniSchwartz:2008}. 
...
As operações de morfologia matemática são realizadas a partir de um elemento estruturante que é utilizado para percorrer a imagem processada. A cada ciclo, é realizada uma operação matemática sobre os \textit{pixels} do elementos estruturante, alterando a imagem \cite{pedriniSchwartz:2008}.
...
Os algoritmos de afinamento de bordas tem como objetivo a remoção de \textit{pixels} redundantes das imagens, produzindo, desta forma, uma simplificação dos objetos da imagem. O principal problema que estes algoritmos enfrentam é determinar com exatidão quais são os \textit{pixels} redundantes da imagem \cite{guilherme:2007}.
...
Na Figura 1.18 são demonstradas as máscaras utilizadas pelo algoritmo, os círculos brancos representam \textit{pixels} de valor zero, os círculos pretos representam os \textit{pixels} de valor 1 e os \(X\) representam os \textit{pixels} com valores irrelevantes.
...
\item Repetir os passos 1 e 2 para todos os \textit{pixels} da imagem que se encaixem na máscara M1;
\item Repetir os passos 1, 2 e 3 para as máscaras M2, M3 e M4;
\item Apagar todos os \textit{pixels} que estejam marcados para serem apagado;
...
\item Ao menos um dos \textit{pixels} da primeira máscara deve ser branco;
